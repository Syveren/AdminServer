 ...
 * @author kirio
 */
public class AddTheaterToExistedProjectDialog extends Window{
   ...
   TheaterGrid grid ;
   int current_project;
   public AddTheaterToExistedProjectDialog(final int project_id) {
            setModal(true);
        ...
        TextButton but_add = new TextButton("Добавить");
        ...
        but_add.addSelectHandler(new SelectEvent.SelectHandler() {
                ...
                public void onSelect(SelectEvent event) {
                        ...
                        GWTServiceAsync.instance.addTheatersToProject(project_id, new ArrayList(checkedItems.values()),
                                new CallbackWithFailureDialog<Void>("Не удалось добавить кинотеатры") {
                                    
                                    @Override
                                    public void onSuccess(Void result) {
                                        for ( Integer id : checkedItems.keySet())
                                        {
                                            TheaterItem item = grid.grid.getStore().findModelWithKey(String.valueOf(id));
                                            if(item!=null)
                                                grid.grid.getStore().remove(item);
                                        }
                                        onAdd(checkedItems.size());
                                        checkedItems.clear();
            ...
            RpcProxy<MyLoadConfig, PagingLoadResult<TheaterItem>> proxy =   new RpcProxy<MyLoadConfig, PagingLoadResult<TheaterItem>>() {
                ...
                public void load(MyLoadConfig loadConfig, AsyncCallback<PagingLoadResult<TheaterItem>> callback) {
                    if(grid_data_request!=null)
                     ...
                     grid_data_request = GWTServiceAsync.instance.getTheatersCanAddToProject(current_project,totalSize,loadConfig,callback);
